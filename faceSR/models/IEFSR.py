...
class generator(nn.Module):
    ...
    def forward(self, input, style):
        ...
        out = F.upsample(input, scale_factor=1 / 8, mode='bicubic')
        out = self.initial(out)
        out = self.upsample_1(out, style)

        # 8 x 8
        # image = F.upsample(input, scale_factor=1/4, mode='bilinear')
        image = F.upsample(input, scale_factor=1 / 4, mode='bicubic')
        ...
        image = F.upsample(input, scale_factor=1 / 2, mode='bicubic')
